...
## Password Filename (encrypted with AES):
passwordFile = "passwords.pwm"

## Salts Filename (not encrypted):
saltFile = "salts.slt"

## Main Salt Value (for master password):
...
## Number of hash iterations (initialized to 100,000 interations):
...
def Main():
    ...
    if entry in pws: #if entry exists
        ...
        for pwchars in range(SALT_LENGTH): #generate random salt
        ...
        if password == "": #if user wants to generate a password
            for pwchars in range(PASSWORD_LENGTH): #generate random password
